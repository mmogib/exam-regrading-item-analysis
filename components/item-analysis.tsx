...
import { ItemAnalysisHelpDialog } from './item-analysis-help-dialog';
import {
...
export function ItemAnalysisTab() {
  ...
  const handleDataReady = (data: {
    ...
    // Validate correct answers immediately when data is ready
    const validationError = validateCorrectAnswers(data.itemAnalysisData, data.answersData);
    ...
    // Check permutation and set warning if missing
    if (!data.hasPermutation) {
      setWarning('Permutation data not detected. Comprehensive psychometric analysis requires QUESTIONS_MAP format with Permutation column. Only cross-version analysis will be available.');
    } else {
      setWarning(null);
    }
  ...
  const handleCompute = () => {
    ...
    setError(null);
    try {
      // Compute comprehensive analysis if permutation data exists
      if (uploadedData.hasPermutation) {
        const comprehensiveResults = computeComprehensiveItemAnalysis(
          uploadedData.answersData,
          uploadedData.itemAnalysisData,
          uploadedData.numQuestions
        );
        setComprehensiveResults(comprehensiveResults);
      } else {
        setComprehensiveResults(null);
      }

      // Always compute cross-version analysis
      ...
      logError('Error computing analysis:', error);
      setError(`Error computing analysis: ${error.message}`);
      setComprehensiveResults(null);
      setAverageResults([]);
      setCodeAverages([]);
    } finally {
  ...
  const downloadAverageResults = () => {
    ...
    const flattenedResults = averageResults.map((row) => {
      ...
      const codes = Object.keys(row.codeStats).sort(
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <AnalysisDataUploader
        ...
        helpDialog={<ItemAnalysisHelpDialog />}
      />
          ...
          <CardHeader>
            ...
            <CardDescription>
              {uploadedData.hasPermutation
                ? 'Comprehensive psychometric analysis including item difficulty, discrimination, point-biserial correlations, distractor efficiency, test reliability (KR-20), and cross-version comparison.'
                : 'Cross-version analysis including master question statistics and exam version performance. Upload QUESTIONS_MAP format with Permutation column for comprehensive psychometric analysis.'
              }
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <>
          ...
          <Card className="border-2 border-purple-200 dark:border-purple-800">
            <CardHeader className="bg-purple-50 dark:bg-purple-950/30">
              ...
              <CardDescription className="text-purple-700 dark:text-purple-300">
                Performance statistics for each master question across all exam
                versions. Note: The same master question may appear at different
                positions in each exam version (e.g., Master Q1 might be Q7 in
                Code 1, Q12 in Code 2). Position numbers are shown in
                parentheses.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 pt-6">
              <Button
                onClick={downloadAverageResults}
                ...
                <Table>
                  <TableHeader className="bg-slate-100 dark:bg-slate-800">
                    <TableRow>
                      ...
                      {(() => {
                        ...
                        const codes =
                          averageResults.length > 0
                            ? Object.keys(averageResults[0].codeStats).sort(
                                (a, b) => parseInt(a) - parseInt(b)
                              )
                            : [];
                  ...
                  <TableBody>
                    {averageResults.map((row) => {
                      ...
                      return (
                        <TableRow
                          key={row.Master_Question}
                          className="hover:bg-slate-50 dark:hover:bg-slate-900/50"
                        >
                          <TableCell className="font-semibold">
                            Q{row.Master_Question}
                          </TableCell>
                          <TableCell className="text-right">
                            <span
